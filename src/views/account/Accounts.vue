  ...
  section.account-list
      ...
      .box-header
        ...
        .buttons
          el-button(type="primary", size="small", v-if="$permit('accountAdd')", @click="addAccount()")
            ...
            | 新增
      .filters
        el-select(placeholder="角色昵称", v-model="filter.roleId", @change="search()")
    ...
    .table-container
      el-table(:data='accountList', style='width: 100%')
        ...
        el-table-column(prop='roles', label='角色昵称', width='120')
      ...
      el-pagination(@size-change='pageSizeChange', @current-change='pageChange', :current-page='parseInt(filter.page)', :page-sizes="page.sizes", :page-size="parseInt(filter.size)", layout='total, prev, pager, next, jumper, ->, sizes, ->, sizes', :total='parseInt(page.total)')
</template>
