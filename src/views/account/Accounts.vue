    ...
    .table-container
      el-table(:data='accountList', style='width: 100%')
        ...
        el-table-column(label='操作', :fixed="fixed", width='80')
          template(scope="scope")
            .operations
              i.iconfont.icon-qiyong(v-if="!scope.row.enabled", title="启用用户", @click="startAccount(scope.row)")
              i.iconfont.icon-tingyong(v-if="scope.row.enabled", title="停用用户", @click.stop="stopAccount(scope.row)")
              i.iconfont.icon-edit(title="修改用户", @click.stop="editAccount(scope.row)")
